				       ...
				       struct apple_mbox_shared_memory *buffer)
{
	...
	if (buffer->iova < apple_mbox->mmio_shmem->start ||
		...
		dev_warn(
			apple_mbox->dev,
			"coprocessor sent shmem buffer with 0x%zx bytes at 0x%llx outside of the configured region 0x%llx...0x%llx",
			buffer->size, buffer->iova,
			apple_mbox->mmio_shmem->start,
			apple_mbox->mmio_shmem->end);
		goto error;
	}
